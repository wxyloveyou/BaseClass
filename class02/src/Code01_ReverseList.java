...
public class Code01_ReverseList {
    ...
    public static DoubleNode testReverstDoubleList(DoubleNode head) {
        if (head == null) {
            return null;
        }
    ...
    public static boolean checkDoubleListEqual(DoubleNode head1, DoubleNode head2) {
        ...
        if (null1 == null2) { //head1 head2都是null ,返回true
            return true;
        }
        if (null1 != null2) { //一个空，一个非空，返回false
            return false;
        }
        if (head1.last != null || head2.last != null) {//判断是不是都为第一节点，第一个节点的last都是指向null的
            return false;
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            if (!checkLinkedListEqual(node1, back1)) {
                System.out.println("Linkedlist is not equal!");
                break;
            }
            DoubleNode node2 = generateRandomDoubleList(len, value);
            DoubleNode reverse2 = reverseDoubleList(node2);
            DoubleNode back2 = testReverstDoubleList(node2);
            if (!checkDoubleListEqual(reverse2, back2)) {
                System.out.println("Doublelist is not equal!");
                break;
            }
        }
